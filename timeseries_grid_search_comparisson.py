...
def split_data(df, p=.8):
    ...
    print("train data: ",list(train_data["RGB_Image"]))
    print("test data: ",list(test_data["RGB_Image"]))
...
def plot_true_vs_predicted_old(predicted, y, title):
    actual = y
    plt.plot(actual, 'bo')
...
def grid_search(train_data,test_data, target): # Make it return rmse, mae, mape, results_df, predictions
    ...
    for name, model in models:
        if is_keras_model(model):g
            # Compile the Keras model
            model.compile(loss="mean_squared_error", optimizer="adam")
            model.fit(train_data.drop(target, axis=1), train_data[target],verbose=0)
    ...
    for model_name in results:
        ...
        if is_keras_model(model):
            ...
            # Make predictions on the test data
            predictions = model.predict(test_data.drop(target, axis=1), verbose=0)
            ...
            predictions = result.predict(start=start, end=end, exog=test_data.drop(target, axis=1), dynamic=False,verbose=0)
        else:
            # score = model.score(test_data.drop(target, axis=1), test_data[target])
            predictions = model.predict(test_data.drop(target, axis=1))

        plot_true_vs_predicted_old(predictions, test_data[[target, "Week"]], 'test')
        ...
        table_row = [target, model_name, rmse]
        ...
        # print(f"  ({target} Mean absolute percentage error: {mape:2f}%)")
    ...
    print(f"\n({target}) Best model: {best_model_name} {best_mae=:.2f}") #{mape=:.2f}%")

    for table_row in table_rows:
        # calculate percentage from best best_rmse
        # percentage_off = 0
        toatalCount = table_row[2]
        completedCount = best_rmse
        percentage_off = ((toatalCount - completedCount)/toatalCount) * 100
        table_row.append(percentage_off)
        # table_row.append(split_step)
        table.append(table_row)
...
if __name__ == "__main__":
    ...
    # for split_step in [80]:
    # # for split_step in range(10,100,10):
    #     split_step = split_step/100
    ...
    train_data, test_data = split_data(df, .8)
    ...
    for column in df.drop(["ID", "Week", "Variety",
        ...
        # train_data = train_data.drop(["ID", "Week"])
        #
        # test_data = test_data.drop(["ID", "Week"])

        # train_data = train_data[['FreshWeightShoot', 'DryWeightShoot', 'Height', 'Diameter', 'LeafArea']]
        # test_data = test_data[['FreshWeightShoot', 'DryWeightShoot', 'Height', 'Diameter', 'LeafArea']]
    ...
    for table in tables:
        frmt = "html"
        # frmt = "rich text"
        tbl = tabulate(table, headers=[
            'Feature', 'Model', 'RMSE', "Difference from best (%)","Split values"],
                       tablefmt=frmt)
        print(tbl)
...
# results = table
    #
    # # Extract the values for RMSE and Difference from best
    # model_names = [result[1] for result in results]
    # rmse_values = [result[2] for result in results]
    # diff_values = [result[3] for result in results]
    #
    # # split_steps = [split_step for x in range(len(model_names))]
    #
    # # split_step
    #
    #
    ...
    #
    # # Set the y-axis labels to be the split steps
    # ax.set_ylabel('Split Steps')
    #
    # # Plot the rmse values as a scatter plot
    # ax.scatter(model_names, split_steps, c=rmse_values)
    #
    # # Display the plot
    # plt.show()
